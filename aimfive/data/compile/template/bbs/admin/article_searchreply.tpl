...
<div class="wrap J_check_wrap">
	<div class="nav">
		<ul class="cc">
			<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=article'; ?>">帖子管理</a></li>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=article&a=replylist'; ?>">回复管理</a></li>
		</ul>
	</div>
	...
	<form method="post" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=article&a=searchreply'; ?>">
	<div class="search_type cc mb10">
		...
		<button class="btn mr20" type="submit">搜索</button><a class="w J_dialog" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=article&a=replyadvanced'; ?>" title="高级搜索">高级搜索</a>
	</div>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
	
	<form class="J_ajaxForm" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?m=bbs&c=article&a=deletereply'; ?>" method="post">
			<?php  if ($posts) { ?>
	<div class="table_list">
		<table width="100%">
			...
			<tr>
				...
				<td><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','read.php?tid=', rawurlencode($v['tid']),'&fid=', rawurlencode($v['fid']); ?>#<?php echo htmlspecialchars($v['pid'], ENT_QUOTES, 'UTF-8');?>" target="_blank"><?php echo htmlspecialchars($v['content'], ENT_QUOTES, 'UTF-8');?></a></td>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?page=', rawurlencode($_page_i),'&m=bbs&c=article&a=searchreply'; 
  ...
  if ($posts) { ?>
	<div class="btn_wrap">
		<div class="btn_wrap_pd">
			...
			<button class="btn btn_submit J_ajax_submit_btn" type="submit" data-action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','admin.php?isDeductCredit=1&m=bbs&c=article&a=deletereply'; ?>">删除并扣除积分</button>
		</div>
	</div>
	<?php  } ?>
	<input type="hidden" name="csrf_token" value="<?php echo WindSecurity::escapeHTML(Wind::getComponent('windToken')->saveToken('csrf_token')); ?>"/></form>
