...
<div class="tac"> </div>
	<div class="main_wrap">
		<div class="bread_crumb">
			...
			<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=notice'; ?>">通知</a>
		</div>
		<div class="main cc">
...
<div class="content_nav">
	...
	<?php if(!$a){?>
		<li<?php if($_controller == 'message' && !Pw::inArray($_action,array('set','add'))){?> class="current"<?php }?>>
			<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=message'; ?>">私信<?php  if ($loginUser->info['messages']) { ?><em class="core_num id="J_unread_count"><?php echo htmlspecialchars($loginUser->info['messages'], ENT_QUOTES, 'UTF-8');?></em><?php  } ?></a>
		</li>
	<?php }
 if($a =='type'){?>
					...
					?>
							<div class="hd cc J_op_bar" style="display:none;">
								...
								<div class="operate J_notice_op">
									...
									<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=notice&a=delete'; ?>" style="visibility:hidden;" class="btn J_check_op J_check_op_sub">删除</a>
								</div>
							</div>
					...
					?>
							<div class="ft J_op_bar" style="display:none;">
								...
								<div class="operate">
									...
									<a class="btn J_check_op J_check_op_sub" style="visibility:hidden;" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=notice&a=delete'; ?>">删除</a>
								</div>
							</div>
						<?php  } 
  if($totalpage > 1){ ?>
...
?>
	<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=notice&page=', rawurlencode($_page_i); 
 ...
 * 忽略
*/
	var URL_IGNORE = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=notice&a=ignore&ignore=1'; ?>",		//忽略
			URL_UNIGNORE = '<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=notice&a=ignore&ignore=0'; ?>';	//取消忽略 开启回复提醒
			...
			if(data.state == 'success') {
				...
				if(role == 'reply') {
					...
					if(type) {
						...
						$this.replaceWith('<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=message&c=message&a=set'; ?>">查看通知设置</a>')
					}else{
