<div id="J_profile_right" class="content">
	<div class="profile_nav">
		<ul>
			<li class="current"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=right'; ?>">权限</a></li>
		</ul>
	</div>
...
<?php foreach ($myGroups as $_gid) {
	if (!$_gid) continue;
?>
				<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=right&gid=', rawurlencode($_gid); ?>"><?php echo htmlspecialchars($allGroups[$_gid]['name'], ENT_QUOTES, 'UTF-8');?></a></li>
<?php }?>
			</ul>
		</div>
	</div>
...
<?php foreach ($_value['sub'] as $_gid => $_gitem) {?>
				<li><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=right&gid=', rawurlencode($_gid); ?>"><?php echo htmlspecialchars($_gitem['name'], ENT_QUOTES, 'UTF-8');?></a></li>
<?php }?>
			</ul>
		</div>
	</div>
	...
	<div id="J_change_group_pop" class="core_pop_wrap" style="display:none;">
		<div class="core_pop">
			<div class="pop_group_select">
			    <form id="J_change_group_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=right&a=doset'; ?>" method="post">
				<div class="pop_top">
