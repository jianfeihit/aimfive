 ...
 foreach ($list as $id => $item) {?>
			<li>
				...
				<span class="time"><?php echo htmlspecialchars($item['start_time'], ENT_QUOTES, 'UTF-8');?>年</span>
					<?php if($item['major']){?>
				<span class="unit1"  style="padding-right:30px;"><?php echo htmlspecialchars($item['major'], ENT_QUOTES, 'UTF-8');?></span>
				<?php }else{?>
				<span class="unit1"  style="padding-right:30px;">未填写</span>
				<?php }?>
			</li>
	<?php }
  } else { ?>
...
<?php  } ?>
			<li id="J_edu_op_wrap" style="display:none;">
				<form id="J_edu_form" action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=education&a=add'; ?>" method="post">
					...
					<?php }?></select></span>
					<span class="unit">
						<input type="text" class="input length_3 J_plugin_school" data-typeid="3" name="school" readonly="true"  style="width:130px;" />
						<input id="J_edit_id" type="hidden" name="schoolid" value=""/></span>
					<span class="time">
						<select id="J_startyear" class="select_2" name="startYear">
					...
					<input type="text" class="input"  name="major"   style="width:130px;margin-left:10px;"  />
...
<script>
var URL_EDU_ADD = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=education&a=add'; ?>",
	URL_EDU_EDIT = "<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=profile&c=education&a=edit'; ?>";
