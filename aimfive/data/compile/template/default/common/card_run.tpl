 ...
 echo htmlspecialchars($follow2num, ENT_QUOTES, 'UTF-8');?>人<?php }} ?>
  ...
  foreach ($medals as $key => $value) { ?>
	...
	?>
		<div class="J_follow_wrap" style="<?php echo htmlspecialchars($style_f, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=follow&a=delete&uid=', rawurlencode($uid); ?>" class="J_card_follow core_unfollow">取消关注</a></div>
		<div class="J_follow_wrap" style="<?php echo htmlspecialchars($style_unf, ENT_QUOTES, 'UTF-8');?>"><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=my&c=follow&a=add&uid=', rawurlencode($uid); ?>" class="core_follow J_card_follow">关注</a></div>
