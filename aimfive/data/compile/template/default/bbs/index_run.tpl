...
PwHook::display(array($threadList, "runDo"), array('createHtmlAfterSubject',$value), "", $__viewer);
  ...
  if ($value['ifupload']) { ?><span class="posts_icon"><i class="icon_<?php echo htmlspecialchars($uploadIcon[$value['ifupload']], ENT_QUOTES, 'UTF-8');?>" title="<?php echo htmlspecialchars($icon[$uploadIcon[$value['ifupload']]], ENT_QUOTES, 'UTF-8');?>"></i></span><?php  } ?>
					...
					<p>悬赏<span><?php echo htmlspecialchars($value['jf'], ENT_QUOTES, 'UTF-8');?></span>分 浏览<span><?php echo htmlspecialchars($value['hits'], ENT_QUOTES, 'UTF-8');?></span> | 回复<span><?php echo htmlspecialchars($value['replies'], ENT_QUOTES, 'UTF-8');?>| <a href="javascript:click_concerned(<?php echo htmlspecialchars($value['tid'], ENT_QUOTES, 'UTF-8');?>,<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>)">关注</a></span></p>
					</div>
					</li>
					<?php  } ?>
					</ul>
