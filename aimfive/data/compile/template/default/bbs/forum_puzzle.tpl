...
<!--design role="segment" id="linkdemo"/-->
	...
	<div class="middleside1" style="width:570px;padding-top:10px;">
		<div class="paixu" style="width:550px">
			...
			<div class="content_filter" ><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=puzzle&order=postdate'; 
 if($menu=='hot'){?>&type=hot<?php }?>" class="<?php echo htmlspecialchars(Pw::isCurrent($order == 'postdate'), ENT_QUOTES, 'UTF-8');?>">最新发帖</a><span>|</span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=puzzle&order=lastpost'; 
 if($menu=='hot'){?>&type=hot<?php }?>" class="<?php echo htmlspecialchars(Pw::isCurrent($order != 'postdate'), ENT_QUOTES, 'UTF-8');?>">最后回复</a></div>
				<h3>浏览</h3>	
			</div>
				<?php  if ($threadList) { ?>
				<ul class="askSel">
				...
				<li>
					<span style="float:right"><?php echo htmlspecialchars($value['v_type'], ENT_QUOTES, 'UTF-8');?></span>
					<h3>
...
<?php } ?>
					...
					<div class="rightside" style="padding:10px 10px 0 0;">
		<h3 class="baseTitle">类别</h3>
		<div id="se_radio">
		<p><a href="index.php?c=forum&a=puzzle"><input type="radio" name=1 <?php if($menu=='nanti'){?>checked<?php }?>>未解决难题</a></p>
		<p><a href="index.php?c=forum&a=sort"><input type="radio" name=1 <?php if($menu=='fenlei'){?>checked<?php }?>>分类浏览</a></p>
		<p><a href="index.php?c=forum&a=puzzle&type=hot"><input type="radio" name=1 <?php if($menu=='hot'){?>checked<?php }?>>最热讨论</a></p>
		</div>
	</div>
			</div>
		</div>
