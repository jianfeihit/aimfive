...
<?php  } 
  ?>
			...
			<div class="content_filter" ><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=track&order=postdate'; ?>" class="<?php echo htmlspecialchars(Pw::isCurrent($order == 'postdate'), ENT_QUOTES, 'UTF-8');?>">最新发帖</a><span>|</span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=track'; ?>" class="<?php echo htmlspecialchars(Pw::isCurrent($order != 'postdate'), ENT_QUOTES, 'UTF-8');?>">最后回复</a></div>
				<h3>我的追踪</h3>
			</div>
			<?php  if ($threadList) { ?>
				<ul class="askSel">
				<?php  foreach ($threadList as $key => $value) { ?>
					<li id="del_<?php echo htmlspecialchars($value['tid'], ENT_QUOTES, 'UTF-8');?>">
...
PwHook::display(array($threadList, "runDo"), array('createHtmlAfterSubject',$value), "", $__viewer);
  ...
  if ($value['ifupload']) { ?><span class="posts_icon"><i class="icon_<?php echo htmlspecialchars($uploadIcon[$value['ifupload']], ENT_QUOTES, 'UTF-8');?>" title="<?php echo htmlspecialchars($icon[$uploadIcon[$value['ifupload']]], ENT_QUOTES, 'UTF-8');?>"></i></span><?php  } ?>
					...
					<p><?php if($value['jf']){?>悬赏<span><?php echo htmlspecialchars($value['jf'], ENT_QUOTES, 'UTF-8');?></span>分 <?php }?> 浏览<span><?php echo htmlspecialchars($value['hits'], ENT_QUOTES, 'UTF-8');?></span> | 回复<span><?php echo htmlspecialchars($value['replies'], ENT_QUOTES, 'UTF-8');?> | <a href="javascript:out_concerned(<?php echo htmlspecialchars($value['tid'], ENT_QUOTES, 'UTF-8');?>,<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>)">取消关注</a></span></p>
					</div>
					</li>
					<?php  } ?>
					</ul>
