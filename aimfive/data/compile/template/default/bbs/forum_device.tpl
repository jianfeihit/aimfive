...
<?php  } 
  ?>
			...
			<div class="content_filter" ><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=device&order=postdate'; ?>" class="<?php echo htmlspecialchars(Pw::isCurrent($order == 'postdate'), ENT_QUOTES, 'UTF-8');?>">最新发帖</a><span>|</span><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=device'; ?>" class="<?php echo htmlspecialchars(Pw::isCurrent($order != 'postdate'), ENT_QUOTES, 'UTF-8');?>">最后回复</a></div>
				<h3>我的器件</h3>
			</div>
			<?php  if ($threadList) { ?>
				<ul class="askSel" >
...
PwHook::display(array($threadList, "runDo"), array('createHtmlAfterSubject',$value), "", $__viewer);
  ...
  if ($value['ifupload']) { ?><span class="posts_icon"><i class="icon_<?php echo htmlspecialchars($uploadIcon[$value['ifupload']], ENT_QUOTES, 'UTF-8');?>" title="<?php echo htmlspecialchars($icon[$uploadIcon[$value['ifupload']]], ENT_QUOTES, 'UTF-8');?>"></i></span><?php  } ?>
					...
					<div class="touxiangdiv"><span><img class="J_avatar" src="<?php echo htmlspecialchars(Pw::getAvatar($value['lastpost_userid']), ENT_QUOTES, 'UTF-8');?>" data-type="middle"  alt="<?php echo htmlspecialchars($loginUser->username, ENT_QUOTES, 'UTF-8');?>" /></span></div>
					<div class="contdiv">
					<p>最后回复：<a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($value['lastpost_userid'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($value['lastpost_userid']); ?>" target="_blank"><?php echo htmlspecialchars($value['lastpost_username'], ENT_QUOTES, 'UTF-8');?>&nbsp;&nbsp;&nbsp;</a><span><a  aria-label="最后回复时间"><?php  echo Pw::time2str($value['lastpost_time'], 'm-d H:i');?></a></span></p>
					
					</div>
					<div class="clear"></div>
					<div class="date">
					<span class="colorGray">楼主：<a class="J_user_card_show" data-uid="<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>" href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?m=space&uid=', rawurlencode($value['created_userid']); ?>"><?php echo htmlspecialchars($value['created_username'], ENT_QUOTES, 'UTF-8');?>&nbsp;&nbsp;&nbsp;</a><span><?php  echo Pw::time2str($value['created_time'], 'Y-m-d');?></span></span>
					<p><?php if($value['jf']){?>悬赏<span><?php echo htmlspecialchars($value['jf'], ENT_QUOTES, 'UTF-8');?></span>分 <?php }?> 浏览<span><?php echo htmlspecialchars($value['hits'], ENT_QUOTES, 'UTF-8');?></span> | 回复<span><?php echo htmlspecialchars($value['replies'], ENT_QUOTES, 'UTF-8');?> | <a href="javascript:click_concerned(<?php echo htmlspecialchars($value['tid'], ENT_QUOTES, 'UTF-8');?>,<?php echo htmlspecialchars($value['created_userid'], ENT_QUOTES, 'UTF-8');?>)">关注</a></p>
					</div>
					</li>
					<?php  } ?>
					</ul>
...
<?php } ?>
	...
	<?php if($type){?>
		...
		<form  action="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=deldevice'; ?>"  method="post">
		<?php  if ($d_name) { ?>		
		<ul class="listUl">
		<?php  foreach ($d_name as $key => $value) { ?>
			<li>
				<p><?php echo htmlspecialchars($value['v_name']['v_name'], ENT_QUOTES, 'UTF-8');?> &nbsp;<input type="checkbox"   style="float:right"  value="<?php echo htmlspecialchars($value['vid'], ENT_QUOTES, 'UTF-8');?>" name="vid[]"/></p>
			</li>
			<?php  } ?>	
				<input type="hidden"  value="<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>"  name="uid">
			...
			<li><p><a onclick="$('#is_add').show();$('#add_device').val('');">添加器件</a>　　　<a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=device'; ?>">退出管理</a></p></li>
		</ul>
		<?php  } else { ?>
						啊哦，您还没有任何器件信息！
		<?php  } ?>
	...
	<div class="rightside" style="padding:50px 10px 0 0;width:160px;float:right">
		...
		<?php  foreach ($d_name as $key => $value) { ?>
			<li>
				<p><input type="checkbox"   <?php if($value['is_show']==1){?>checked="checked"<?php }?>  onclick="sestate(<?php echo htmlspecialchars($value['vid'], ENT_QUOTES, 'UTF-8');?>,<?php echo htmlspecialchars($value['uid'], ENT_QUOTES, 'UTF-8');?>,<?php echo htmlspecialchars($value['is_show'], ENT_QUOTES, 'UTF-8');?>)"/>&nbsp;<?php echo htmlspecialchars($value['v_name']['v_name'], ENT_QUOTES, 'UTF-8');?></p>
			</li>
			<?php  } ?>	
			<li><p><a href="<?php echo Wind::getComponent('response')->getData('G', 'url', 'base'),'/','index.php?c=forum&a=device&type=edit'; ?>">器件管理</a></p></li>
		</ul>
		<?php  } else { ?>
						啊哦，您还没有任何器件信息！
		<?php  } ?>
