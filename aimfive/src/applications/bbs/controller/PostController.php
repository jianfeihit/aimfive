...
class PostController extends PwBaseController {
	...
	public function beforeAction($handlerAdapter) {
		...
		if($_GET['type']=='fx'){
			$this->setOutput('fabu1', 'menu');
		}else{
			$this->setOutput('fabu', 'menu');
		}		
	...
	public function run() {
		...
		if (!$this->loginUser->isExists()) {
		 ...
		 $jf= $this->getThreadCredit()->getCredit($this->loginUser->uid);
		$this->setOutput($jf['credit2'],'jfa');
		$this->runHook('c_post_run', $this->post);
		$this->setOutput($this->getInput('type'),'t_type');
		...
		if($topictype==1){
			if(!$jf){
					...
					if($jf>$jfinfo['credit2']){
						...
						$sum = $jfinfo['credit2']-$jf;
						$re = $this->getThreadCredit()->savCreditjf($this->loginUser->uid,$sum);
						$info =$this->loginUser->username."在版块主题发布主题悬赏$jf;积分变化【积分:- $jf 】";
						$this->getThreadCredit()->addLogCreditjf($this->loginUser->uid,$this->loginUser->username,$info,$time=time());
					}
			}
		}else if($topictype==2){
			$jfinfo=$this->getThreadCredit()->getCredit($this->loginUser->uid);
			$sum = $jfinfo['credit2']+5;
			$re = $this->getThreadCredit()->savCreditjf($this->loginUser->uid,$sum);
			$info =$this->loginUser->username."在版块主题发布分享;积分变化【积分:+5 】";
			$this->getThreadCredit()->addLogCreditjf($this->loginUser->uid,$this->loginUser->username,$info,$time=time());
	...
	public function modifyAction() {
		...
		$info = $this->post->getInfo();
		$v_type = $this->getVender()->getvname($info['v_type']);
		$jf= $this->getThreadCredit()->getCredit($this->loginUser->uid);
		$this->setOutput($jf['credit2'],'jfa');
		$this->setTemplate('post_run');
		$this->setOutput($info['vender'], 'vender');
		$this->setOutput($info['jf'], 'jf');
		$this->setOutput($v_type['v_name'], 'v_type');
	...
	public function domodifyAction() {
		$tid = $this->getInput('tid');
		$pid = $this->getInput('pid');
		list($title, $content, $topictype, $jf,$subtopictype, $reply_notice, $hide) = $this->getInput(array('atc_title', 'atc_content', 'topictype','jf', 'sub_topictype', 'reply_notice', 'hide'), 'post');
		$pwPost = $this->post;
