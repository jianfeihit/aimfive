...
//查询筛选器件的方法
	protected function _fetchA3($ids, $index = '', $fetchMode = 0,$uid) {
		$sql = $this->_bindSql('SELECT * FROM %s WHERE %s IN %s  %s', $this->getTable(), $this->_pk, $this->sqlImplode($ids)," AND created_userid=$uid AND vtp_is_show=0 ");
		$rst = $this->getConnection()->query($sql);
		return $rst->fetchAll($index, $fetchMode);
	}
	//修改后查询提问或者分享的方法
		protected function _fetch_tw($ids,$type,$uid='', $index = '', $fetchMode = 0) {
				$ids=implode(",",$ids);
				$sql = $this->_bindSql('SELECT * FROM %s WHERE %s IN %s ', $this->getTable()," topic_type=$type AND created_userid=$uid  AND tid ","($ids)");
				$rst = $this->getConnection()->query($sql);
				return $rst->fetchAll($index, $fetchMode);
		}
	
//查询提问难题方法
		protected function _fetch_le($ids,$type, $index = '', $fetchMode = 0) {
				$ids=implode(",",$ids);
				$sql = $this->_bindSql('SELECT * FROM %s WHERE %s IN %s ', $this->getTable()," topic_type=$type AND is_ok=0  AND tid ","($ids)");
				$rst = $this->getConnection()->query($sql);
				return $rst->fetchAll($index, $fetchMode);
		}
//查询热门难题方法
		protected function _fetch_hist($ids,$type, $index = '', $fetchMode = 0) {
				$ids=implode(",",$ids);
				$sql = $this->_bindSql('SELECT * FROM %s WHERE %s IN %s  ORDER BY hits DESC', $this->getTable()," tid ","($ids)");
				$rst = $this->getConnection()->query($sql);
				return $rst->fetchAll($index, $fetchMode);
		}
