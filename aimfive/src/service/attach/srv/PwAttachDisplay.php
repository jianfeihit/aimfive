 ...
 * @version $Id: PwAttachDisplay.php 24304 2013-01-28 05:27:41Z yanchixia $
...
class PwAttachDisplay {
	...
	public function parsePicHtml($att) {
		...
		$html .= '<p>图片：' . $att['name'];
		if ($this->isAdmin) {
			$html .= '<a class="J_read_img_del w" href="' . WindUrlHelper::createUrl('bbs/attach/delete', array('aid' => $att['aid'])) . '">[删除]</a>';
		}
		$html .= '</p></div>';
		$html .= $att['img'];
		$html .= '</span>';
