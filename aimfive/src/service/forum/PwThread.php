...
class PwThread {
	 ...
	 * @return array
	 */
	public function fetchThread($tids,$type='',$uid='', $fetchmode = self::FETCH_MAIN) {
		if (empty($tids) || !is_array($tids)) return array();
		if($type=='a3'){
			return $this->_getThreadDao($fetchmode)->fetchThreadA3($tids,$uid);
		}else{
...
//后台删除帖子的方法
	...
	//查询回复的方法
		public function fetchThreada($tids, $fetchmode = self::FETCH_MAIN) {
		if (empty($tids) || !is_array($tids)) return array();
		return $this->_getThreadDao($fetchmode)->fetchThreada($tids);
	}
