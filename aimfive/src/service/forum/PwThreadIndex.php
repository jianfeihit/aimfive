...
class PwThreadIndex {
	...
	public function fetch($limit, $offset, $order = 'lastpost') {
		$result = $this->_getDao()->fetch1($limit, $offset, $order);
	 ...
	 * 
	 * 查询回复帖子的ID
	 * 增加了_getDao_back()方法,在PwPostExpandDao类中添加了backId()方法(按规则查询出了用户回复的帖子ID)
	 * @param $fids
	 * @param $limit
	 * @param $offset
	 * @param $order
	 * @param $uid
	 */
	public function fetchInFid_back($fids='2', $limit, $offset, $order = 'lastpost',$uid) {

		if (empty($fids) || !is_array($fids)) return array();
		
		$result = $this->_getDao_back()->backId($uid,$fids, $limit, $offset, $order);
	...
	protected function _getDao_back() {
