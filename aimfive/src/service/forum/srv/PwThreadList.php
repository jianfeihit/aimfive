...
class PwThreadList extends PwBaseHookService {
	...
	*/
	public function execute(PwThreadDataSource $ds,$type='',$puzzle='',$hits='',$uid='') {
		...
		if($puzzle==1){
			$threaddb = $ds->getData_le($limit, $start);
		}else{
			$threaddb = $ds->getData($limit, $start);
		}
		if($hits==1){
				$thre_hist = $ds->getData_hist($limit, $start);
				$this->thre_hist = $thre_hist;
		}
	 ...
	 * @param $ds
	 * @param $uid
	 * 主要更改了getData_back()到PwMyForumThread类中
	 */
	public function execute_back(PwThreadDataSource $ds,$uid,$track='',$firend='') {
		$this->_ds = $ds;
		$this->total = $ds->getTotal();
		$this->maxPage = ceil($this->total / $this->perpage);
		$this->_maxPage > 0 && $this->maxPage = min($this->maxPage, $this->_maxPage);
		...
		$threaddb = $ds->getData_back($limit, $start,$uid,$track,$firend);
		$this->runDo('initData', $threaddb);
		foreach ($threaddb as $key => $value) {
			$threaddb[$key] = $this->bulidThread($value);
		}
	...
	public function getList_list() {
		return $this->thre_hist;
	}
