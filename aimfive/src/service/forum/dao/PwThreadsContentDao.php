...
class PwThreadsContentDao extends PwBaseDao {
	...
	public function addThread($fields) {
		$object= $this->getNewTableDao();		
		$v_type=$object->check_vtype($fields['v_type']);
		if(empty($v_type)){
				$v_result =  $object->add_vtype($fields['v_type']);			//增加新器件型号,并返回插入ID
				$fields['v_type']=$v_result;
				if (!$tid = $this->getBaseInstance()->addThread($fields)) {
					return false;
				}
				if(!empty($v_result)){$object->add_vtype_my($v_result,$fields['created_userid'],$tid);}	//更新自己的器件表
		}else{
				$fields['v_type']=$v_type['id'];
				if (!$tid = $this->getBaseInstance()->addThread($fields)) {
					return false;
				}
				$my_result =  $object->check_my($v_type['id'],$fields['created_userid'],$tid);						 //检查自己的器件表
				if(empty($my_result)){$object->add_vtype_my($v_type['id'],$fields['created_userid'],$tid);}	//更新自己的器件表
		}
		array_splice($fields,2,3);
		$fields['tid'] = $tid;
	...
	protected function getNewTableDao() {
